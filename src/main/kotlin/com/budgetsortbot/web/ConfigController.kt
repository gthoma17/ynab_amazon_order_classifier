...
class ConfigController(
    ...
    @PutMapping("/keys")
    fun updateKeys(
        @RequestBody request: ApiKeysRequest,
    ): ResponseEntity<Void> {
    ...
    fun getCategories(): List<CategoryRuleResponse> = configService.getAllCategoryRules().map { it.toResponse() }

    @PutMapping("/categories")
    fun updateCategories(
        @RequestBody rules: List<CategoryRuleRequest>,
    ): ResponseEntity<Void> {
        val entities =
            rules.map { req ->
                CategoryRule(
    ...
    private fun CategoryRule.toResponse() =
    ...
    fun updateProcessingConfig(
        @RequestBody request: ProcessingConfigRequest,
    ): ResponseEntity<Void> {
    ...
    @PostMapping("/dry-run")
    fun triggerDryRun(
        @RequestBody(required = false) request: DryRunRequest?,
    ): List<DryRunResultResponse> {
        val startFromDate =
            request?.startFromDate?.let {
    ...
    fun getDryRunResults(): List<DryRunResultResponse> = dryRunService.getResults().map { it.toResponse() }

    // ── Helpers ───────────────────────────────────────────────────────────────

    private fun ScheduleConfig.toDto() =
        ScheduleConfigDto(
            type = type.name,
