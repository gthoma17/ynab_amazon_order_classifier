...
class EmailIngestionService(
    ...
    companion object {
        ...
        private val AMOUNT_PATTERN =
            Regex("""(?:Order Total|Grand Total|Total|Subtotal)\s*:?\s*\$?\s*([\d,]+(?:\.\d+)?)""", RegexOption.IGNORE_CASE)
    ...
    fun ingest() {
        ...
        try {
            ...
            val sinceDate =
                if (startFromInstant != null && startFromInstant.isAfter(lastSyncInstant)) {
                    startFromInstant
                } else {
                    lastSyncInstant
                }
