...
class GeminiProviderTest {
    ...
    fun `classify sends POST to Gemini and returns trimmed category ID from response`() {
        mockServer
            .expect(requestTo("${GeminiProvider.BASE_URL}/models/${GeminiProvider.MODEL}:generateContent?key=gemini-api-key"))
    ...
    fun `classify includes items and category rules in prompt`() {
        mockServer
            .expect(requestTo("${GeminiProvider.BASE_URL}/models/${GeminiProvider.MODEL}:generateContent?key=my-key"))
            .andExpect(method(HttpMethod.POST))
            .andExpect { request ->
